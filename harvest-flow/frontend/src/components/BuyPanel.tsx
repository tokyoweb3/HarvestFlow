...
const BuyPanel: React.FC<BuyPanelProps> = ({ nftContractAddress }) => {
  ...
  useEffect(() => {
    mainController
      .getDetailedNftContract(nftContractAddress)
      .then((details) => {
  ...
  const buyNft = (amountToBuy: number) => {
    ...
    mainController.buyNft(nftContractAddress, amountToBuy, BigInt(nftDetails.price)).then(() => {
      mainController
        .getDetailedNftContract(nftContractAddress)
        .then((details) => {
          setNftDetails(details);
  ...
  return (
    <Container className={"borderAll"} sx={{ width: "400px" }} disableGutters>
      <Stack direction="row">
        <div style={{ width: "50%" }} className="underline uppercase">
          {/* TODO: handle phases*/}
          Phase: Allow List
        </div>
        <div>Ending in {endingIn}</div>
      </Stack>
      ...
      <Container>
        <Stack alignItems={"center"} direction="column">
          <Container sx={{ width: "50%", margin: "10px" }}>
            {nftDetails && (
              <ProgressBar
                total={Number(nftDetails.supplyCap)}
                progress={Number(nftDetails.mintedAmount)}
              />
            )}
          </Container>
        ...
        >
          ...
          <Stack direction={"column"} alignItems="start">
            ...
            <span>
              {nftDetails ? ethers.utils.formatEther(nftDetails.price) : "----"}{" "}
              DAI
            </span>
            {/*TODO: actual values*/}
